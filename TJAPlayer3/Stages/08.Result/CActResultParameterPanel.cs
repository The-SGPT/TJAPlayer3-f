...
namespace TJAPlayer3
{
	internal class CActResultParameterPanel : CActivity
	{
		...
		public override void On活性化()
		{
			...
			this.ctCrown用 = new CCounter(0, 255, 3, TJAPlayer3.Timer);
            ...
            for (int i = 0; i < TJAPlayer3.ConfigIni.nPlayerCount; i++)
			{
				...
				else
				{
					if (CrownState[i] != 0 && TJAPlayer3.Tx.Crown_t != null)
					{
						...
						TJAPlayer3.Tx.Crown_t.vc拡大縮小倍率.X = ((this.ctCrown用.n終了値 - this.ctCrown用.n現在の値) / 255f) + 1.0f;
						TJAPlayer3.Tx.Crown_t.vc拡大縮小倍率.Y = ((this.ctCrown用.n終了値 - this.ctCrown用.n現在の値) / 255f) + 1.0f;
