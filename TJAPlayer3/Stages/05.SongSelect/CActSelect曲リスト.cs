...
namespace TJAPlayer3
{
	internal class CActSelect曲リスト : CActivity
	{
		...
		public override int On進行描画()
		{
			...
			for (int i = 0; i < TJAPlayer3.Tx.SongSelect_Box_Center_Header_Genre.Length; i++)
				if(TJAPlayer3.Tx.SongSelect_Box_Center_Header_Genre[i] != null)
					TJAPlayer3.Tx.SongSelect_Box_Center_Header_Genre[i].vc拡大縮小倍率 = new Vector3(this.ctバー展開用タイマー.n現在の値 / 100f);
			//-----
			...
			if (!this.b登場アニメ全部完了)
			{
				if ((this.n現在のスクロールカウンタ == 0) && (ctバー展開ディレイ用タイマー.b終了値に達した))
				{
					if (TJAPlayer3.Tx.SongSelect_Box_Center_Genre[nnGenreBack] != null && TJAPlayer3.Tx.SongSelect_Box_Center_Header_Genre[nnGenreBack] != null && this.r現在選択中の曲.eノード種別 != C曲リストノード.Eノード種別.BACKBOX && this.r現在選択中の曲.eノード種別 != C曲リストノード.Eノード種別.RANDOM)
					{
						...
						{
							TJAPlayer3.Tx.SongSelect_Box_Center_Genre[nnGenreBack].t2D描画(TJAPlayer3.app.Device, 645 - (int)(TJAPlayer3.Tx.SongSelect_Box_Center_Genre[nnGenreBack].szテクスチャサイズ.Width / 2 * ctバー展開用タイマー.n現在の値 / 100.0), TJAPlayer3.Skin.SongSelect_Overall_Y);
							if (TJAPlayer3.Tx.SongSelect_Box_Center_Text_Genre[nnGenreBack] != null)
								TJAPlayer3.Tx.SongSelect_Box_Center_Text_Genre[nnGenreBack].t2D描画(TJAPlayer3.app.Device, 645 - (int)(TJAPlayer3.Tx.SongSelect_Box_Center_Text_Genre[nnGenreBack].szテクスチャサイズ.Width / 2 * ctバー展開用タイマー.n現在の値 / 100.0), TJAPlayer3.Skin.SongSelect_Overall_Y);
							TJAPlayer3.Tx.SongSelect_Box_Center_Header_Genre[nnGenreBack].t2D描画(TJAPlayer3.app.Device, 645 - (int)(TJAPlayer3.Tx.SongSelect_Box_Center_Header_Genre[nnGenreBack].szテクスチャサイズ.Width / 2 * ctバー展開用タイマー.n現在の値 / 100.0), TJAPlayer3.Skin.SongSelect_Overall_Y + TJAPlayer3.Skin.SongSelect_Box_Center_Header_Y_Diff - 69 + TJAPlayer3.Tx.SongSelect_Box_Center_Header_Genre[nnGenreBack].szテクスチャサイズ.Height / 2 - (int)(TJAPlayer3.Tx.SongSelect_Box_Center_Header_Genre[nnGenreBack].szテクスチャサイズ.Height / 2 * ctバー展開用タイマー.n現在の値 / 100.0));
						}
						else if (TJAPlayer3.Tx.SongSelect_Bar_Center != null)
							TJAPlayer3.Tx.SongSelect_Bar_Center.t2D描画(TJAPlayer3.app.Device, 645 - (int)(TJAPlayer3.Tx.SongSelect_Bar_Center.szテクスチャサイズ.Width / 2 * ctバー展開用タイマー.n現在の値 / 100.0), TJAPlayer3.Skin.SongSelect_Overall_Y);
					}
					else if (TJAPlayer3.Tx.SongSelect_Bar_Center != null)
						TJAPlayer3.Tx.SongSelect_Bar_Center.t2D描画(TJAPlayer3.app.Device, 645 - (int)(TJAPlayer3.Tx.SongSelect_Bar_Center.szテクスチャサイズ.Width / 2 * ctバー展開用タイマー.n現在の値 / 100.0), TJAPlayer3.Skin.SongSelect_Overall_Y);
				...
				for (int i = 0; i < 13; i++)    // パネルは全13枚。
				{
					if (this.ct登場アニメ用[i].n現在の値 >= 0)
					{
								...
								if (TJAPlayer3.Tx.SongSelect_Box_Center_Genre[nnGenreBack] != null && TJAPlayer3.Tx.SongSelect_Box_Center_Header_Genre[nnGenreBack] != null) {
									if (this.ttk選択している曲の曲名 != null)
									{
										if (this.r現在選択中の曲.strジャンル == "" || this.r現在選択中の曲.strジャンル == "段位道場" || this.e曲のバー種別を返す(this.r現在選択中の曲) != Eバー種別.Box || this.e曲のバー種別を返す(this.r現在選択中の曲) == Eバー種別.Box && nStrジャンルtoNum(this.r現在選択中の曲.strジャンル) == 0)
											ResolveTitleTexture(this.ttk選択している曲の曲名).t2D描画(TJAPlayer3.app.Device, 750, TJAPlayer3.Skin.SongSelect_Overall_Y + TJAPlayer3.Skin.SongSelect_Box_Center_Header_Y_Diff + 23);
									}
								}
						...
						else
						{
							...
							this.tジャンル別選択されていない曲バーの描画(this.ptバーの座標[nパネル番号].X, TJAPlayer3.Skin.SongSelect_Overall_Y, this.stバー情報[nパネル番号].strジャンル, this.stバー情報[nパネル番号].eバー種別,this.stバー情報[nパネル番号].eノード種別);
							...
							bool DanJudge = false;
							if (TJAPlayer3.Tx.Crown_t != null && TJAPlayer3.Tx.DanC_Crown_t != null && (i != 5 || ! ctバー展開ディレイ用タイマー.b終了値に達した) && this.stバー情報[i].eバー種別 == Eバー種別.Score)
							{
			...
			else
			{
				...
				for (int i = 0; i < 13; i++)    // パネルは全13枚。
				{
...

					{
						...
						//-----------------
						if (n現在のスクロールカウンタ != 0)
							this.tジャンル別選択されていない曲バーの描画(xAnime, TJAPlayer3.Skin.SongSelect_Overall_Y, this.stバー情報[nパネル番号].strジャンル, this.stバー情報[nパネル番号].eバー種別,this.stバー情報[nパネル番号].eノード種別);
						else if (n見た目の行番号 != 5 || !(ctバー展開ディレイ用タイマー.b終了値に達した))
							this.tジャンル別選択されていない曲バーの描画(xAnime, TJAPlayer3.Skin.SongSelect_Overall_Y, this.stバー情報[nパネル番号].strジャンル, this.stバー情報[nパネル番号].eバー種別,this.stバー情報[nパネル番号].eノード種別);
						...
						//-----------------	
						bool DanJudge = false;
						if (TJAPlayer3.Tx.Crown_t != null && TJAPlayer3.Tx.DanC_Crown_t != null && (n見た目の行番号 != 5 || !ctバー展開ディレイ用タイマー.b終了値に達した) && this.stバー情報[nパネル番号].eバー種別 == Eバー種別.Score)
						{
						...
						if (TJAPlayer3.stage選曲.n現在選択中の曲の難易度[0] <= 4)
						{
							for (int i = 0; i < 4; i++)
							{
								if (TJAPlayer3.Tx.Crown_t != null && this.r現在選択中のスコア.譜面情報.n王冠[i] >= 0 && this.r現在選択中のスコア.譜面情報.n王冠[i] <= 3)
								{
									if (i == 3 && TJAPlayer3.stage選曲.n現在選択中の曲の難易度[0] == 4)
									{
										TJAPlayer3.Tx.Crown_t.t2D描画(TJAPlayer3.app.Device, i * 60 + 482, TJAPlayer3.Skin.SongSelect_Overall_Y + 57, new Rectangle((this.r現在選択中のスコア.譜面情報.n王冠[4]) * 100, 0, 100, 100));
									}
				...
				for (int i = 0; i < 13; i++)    // パネルは全13枚。
				{
					...
					if ((i == 5) && (this.n現在のスクロールカウンタ == 0) && (ctバー展開ディレイ用タイマー.b終了値に達した))
					{
		...
		private void tジャンル別選択されていない曲バーの描画(int x, int y, string strジャンル,Eバー種別 Eバー,C曲リストノード.Eノード種別 Eノード)
		{
				...
				else
				{
					switch (strジャンル)
					{
						case "J-POP":
							#region [ J-POP ]
							//-----------------
							if (TJAPlayer3.Tx.SongSelect_Bar_Box_Genre[1] != null)
								TJAPlayer3.Tx.SongSelect_Bar_Box_Genre[1].t2D描画(TJAPlayer3.app.Device, x, y - boxsabun);
							else if (TJAPlayer3.Tx.SongSelect_Bar_Genre[1] != null)
						...
						case "アニメ":
							#region [ アニメ ]
							//-----------------
							if (TJAPlayer3.Tx.SongSelect_Bar_Box_Genre[2] != null)
								TJAPlayer3.Tx.SongSelect_Bar_Box_Genre[2].t2D描画(TJAPlayer3.app.Device, x, y - boxsabun);
							else if (TJAPlayer3.Tx.SongSelect_Bar_Genre[2] != null)
						...
						case "ゲームミュージック":
							#region [ ゲーム ]
							//-----------------
							if (TJAPlayer3.Tx.SongSelect_Bar_Box_Genre[3] != null)
								TJAPlayer3.Tx.SongSelect_Bar_Box_Genre[3].t2D描画(TJAPlayer3.app.Device, x, y - boxsabun);
							else if (TJAPlayer3.Tx.SongSelect_Bar_Genre[3] != null)
						...
						case "ナムコオリジナル":
							#region [ ナムコオリジナル ]
							//-----------------
							if (TJAPlayer3.Tx.SongSelect_Bar_Box_Genre[4] != null)
								TJAPlayer3.Tx.SongSelect_Bar_Box_Genre[4].t2D描画(TJAPlayer3.app.Device, x, y - boxsabun);
							else if (TJAPlayer3.Tx.SongSelect_Bar_Genre[4] != null)
						...
						case "クラシック":
							#region [ クラシック ]
							//-----------------
							if (TJAPlayer3.Tx.SongSelect_Bar_Box_Genre[5] != null)
								TJAPlayer3.Tx.SongSelect_Bar_Box_Genre[5].t2D描画(TJAPlayer3.app.Device, x, y - boxsabun);
							else if (TJAPlayer3.Tx.SongSelect_Bar_Genre[5] != null)
						...
						case "バラエティ":
							#region [ バラエティ ]
							//-----------------
							if (TJAPlayer3.Tx.SongSelect_Bar_Box_Genre[6] != null)
								TJAPlayer3.Tx.SongSelect_Bar_Box_Genre[6].t2D描画(TJAPlayer3.app.Device, x, y - boxsabun);
							else if (TJAPlayer3.Tx.SongSelect_Bar_Genre[6] != null)
						...
						case "どうよう":
							#region [ どうよう ]
							//-----------------
							if (TJAPlayer3.Tx.SongSelect_Bar_Box_Genre[7] != null)
								TJAPlayer3.Tx.SongSelect_Bar_Box_Genre[7].t2D描画(TJAPlayer3.app.Device, x, y - boxsabun);
							else if (TJAPlayer3.Tx.SongSelect_Bar_Genre[7] != null)
						...
						case "VOCALOID":
							#region [ ボカロ ]
							//-----------------
							if (TJAPlayer3.Tx.SongSelect_Bar_Box_Genre[8] != null)
								TJAPlayer3.Tx.SongSelect_Bar_Box_Genre[8].t2D描画(TJAPlayer3.app.Device, x, y - boxsabun);
							else if (TJAPlayer3.Tx.SongSelect_Bar_Genre[8] != null)
						...
						default:
							#region [ その他の場合 ]
							//-----------------
							if (TJAPlayer3.Tx.SongSelect_Bar_Box_Genre[0] != null)
								TJAPlayer3.Tx.SongSelect_Bar_Box_Genre[0].t2D描画(TJAPlayer3.app.Device, x, y - boxsabun);
							else if (TJAPlayer3.Tx.SongSelect_Bar_Genre[0] != null)
