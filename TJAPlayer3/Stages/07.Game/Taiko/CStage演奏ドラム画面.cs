		...
		protected override void t進行描画_チップ_Taiko( CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip, int nPlayer )
        {
            ...
            if( pChip.b可視 )
            {
                if( !pChip.bHit || pChip.bShow )
				{
                    ...
                    {
                        x = (int)( ( ( ( pChip.n発声時刻ms ) - ( pChip.n発声時刻ms - pChip.nノーツ移動開始時刻ms ) ) * pChip.dbBPM * pChip.dbSCROLL * ( this.act譜面スクロール速度.db現在の譜面スクロール速度.Drums + 1.0 ) ) / 502.8594 ); // 2020.04.18 Mr-Ojii rhimm様のコードを参考にばいそくの計算の修正
                    }
                    else
                    {
                    ...
                    if( pChip.dbSCROLL_Y != 0.0 )
                    {
                        ...
                        if( TJAPlayer3.ConfigIni.eScrollMode == EScrollMode.Normal )
                            y += (int) ( ( ( pChip.n発声時刻ms - CSound管理.rc演奏用タイマ.n現在時刻 ) * pChip.dbBPM * pChip.dbSCROLL_Y * ( this.act譜面スクロール速度.db現在の譜面スクロール速度.Drums + 1.0 ) ) / 502.8594 ); // 2020.04.18 Mr-Ojii rhimm様のコードを参考にばいそくの計算の修正
		...
		protected override void t進行描画_チップ_Taiko連打( CConfigIni configIni, ref CDTX dTX, ref CDTX.CChip pChip, int nPlayer )
        {
            ...
            if( pChip.b可視 )
            {
                if( pChip.nチャンネル番号 >= 0x15 && pChip.nチャンネル番号 <= 0x18 )
                {
                    ...
                    {
                        nノート座標 = (int)( ( ( ( pChip.n発声時刻ms ) - ( pChip.n発声時刻ms - pChip.nノーツ移動開始時刻ms ) ) * pChip.dbBPM * pChip.dbSCROLL * ( this.act譜面スクロール速度.db現在の譜面スクロール速度.Drums + 1.0 ) ) / 502.8594 );// 2020.04.18 Mr-Ojii rhimm様のコードを参考にばいそくの計算の修正
                        nノート末端座標 = (int)( ( ( pChip.nノーツ終了時刻ms - ( pChip.n発声時刻ms - pChip.nノーツ移動開始時刻ms ) ) * pChip.dbBPM * pChip.dbSCROLL * ( this.act譜面スクロール速度.db現在の譜面スクロール速度.Drums + 1.0 ) ) / 502.8594 );// 2020.04.18 Mr-Ojii rhimm様のコードを参考にばいそくの計算の修正
                    }
                    else
                    {
                        nノート座標 = 0;
                ...
                if( pChip.nチャンネル番号 == 0x18 )
                {
                    ...
                    {
                        nノート座標 = (int)( ( ( pChip.n発声時刻ms - ( n先頭発声位置 - pChip.nノーツ移動開始時刻ms ) ) * pChip.dbBPM * pChip.dbSCROLL * ( this.act譜面スクロール速度.db現在の譜面スクロール速度.Drums + 1.0 ) ) / 502.8594 );// 2020.04.18 Mr-Ojii rhimm様のコードを参考にばいそくの計算の修正
                    }
                    else
                    {
                        nノート座標 = 0;
