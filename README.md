...
ビルドとしては、.NET Framework 4.5と.NET Framework 4.8がありますが、.NET Framework 4.8を推奨しています。  
.NET Framework 4.5版のビルド側には必要ファイルがそろっていませんので、  

+ dllフォルダ
+ TJAPlayer3-f.exe
+ TJAPlayer3-f.exe.config

以外をnet48フォルダからコピーしてください。    
.NET Framework 4.5版は基本的に動作確認をしていませんので、動かないかも…  
どちらのビルド版でも、動作保証はできかねますので、あしからず。
